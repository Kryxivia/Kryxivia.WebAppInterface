...
function useHasAccess(account: string | null | undefined, winners: Winners) {
            ...
            <div className="app-c">
                ...
                <div className={!hasAccess ? "" : "btmi"}>
                    <button className={!hasAccess ? "bt" : "bt bt-p"} onClick={(e) => mint(e)} disabled={!hasAccess}>
                        {!account &&  <span>Connect to mint</span>}
                        {account && !hasAccess && <span>You're not part of the alpha</span>}
                        {account && hasAccess && (
                            <>
